...
def main(mesh_size, folder):
    ...
    dofs = np.array([vec_bdm1.ndof(sd), bdm1.ndof(sd), vec_p0.ndof(sd), l1.ndof(sd)])
    ...
    force_r = proj_l1.T @ M_p1 @ p1.interpolate(sd, f_r)
...
if __name__ == "__main__":
    ...
    folder = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "results_case7_not_lump"
    )
